...
python gamemode.py --character shen_sama    # boot Shen Sama (or: cookie, holly, amaris)
python gamemode.py --character cookie       # another existing campaign
```
...
The `--character` flag reads a manifest from `Kenji/Game init files/manifests/<name>.json`. Ship manifests include `kenji`, `cookie`, `amaris`, `holly`, and `shen_sama`; new characters get one auto-generated by the wizard.
...
| **`Cookie/`** | Cookie — heartland Ankuspawn bard/dancer starter arc (`Chapters/` + `character_world_state.json`). Launch: `--character cookie`. |
| **`Holly/`** | Holly — winter-elf inventor arc (`Chapters/` + `character_world_state.json`). Launch: `--character holly`. |
| **`Shen_Sama/`** | Shen Sama — Ankuspawn true dragon, Dragonspine → Highford road “quiet monster” arc (`Chapters/` + `character_world_state.json`). Launch: `--character shen_sama`. |
