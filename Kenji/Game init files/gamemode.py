...
def gamemode(character: str = "kenji", player_action: str = "") -> dict:
    ...
    if combat and combat.active:
        ...
        if player_action and current_name != pc_name and current_name:
            ...
            if end_reason:
                ...
                if end_reason == "ALL_ENEMIES_DEAD":
                    ...
                    print(f"  >>> {pc_name} IS DOWN — begin death saves <<<")
                print(f"  {'=' * 40}")

            # Update current_name after advancing
            if combat and combat.active and 0 <= combat.turn_idx < len(combat.order):
                current_name = combat.order[combat.turn_idx][1]

        # If player submitted an action and it's the PC's turn, PROCESS IT
        if player_action and combat and combat.active and current_name == pc_name:
            # Classify the player's combat action
            pc_fighter = combat.fighters[pc_name]
            pc_abilities = []
            for ab_key, ab in pc_fighter.abilities.items():
                pc_abilities.append({
                    "name": ab.name,
                    "action_type": getattr(ab, "action_type", "action"),
                    "description": getattr(ab, "description", ""),
                    "slot_level": getattr(ab, "slot_level", 0),
                    "max_charges": getattr(ab, "max_charges", 0),
                    "charges": getattr(ab, "charges", 0),
                })

            action_info = _classify_action(player_action, pc_abilities)

            print(f"\n{'=' * 60}")
            print(f"COMBAT — PROCESSING {pc_name.upper()}'S ACTION")
            print(f"{'=' * 60}")
            print(f"  Declared: \"{player_action}\"")
            print(f"  Parsed as: {action_info['type']} (costs {action_info['action_cost']})")
            print(f"  Economy before: {pc_fighter.action_economy_line()}")

            # Execute the action through the engine
            results = _process_pc_combat_turn(combat, pc_name, action_info)
            _print_combat_action_results(results)

            print(f"  Economy after: {pc_fighter.action_economy_line()}")

            # If action was processed and no more actions available, or end_turn
            end_this_turn = action_info["type"] == "end_turn"
            if not end_this_turn:
                # Check if PC still has actions left
                has_action = not pc_fighter.action_used
                has_bonus = not pc_fighter.bonus_action_used
                if not has_action and not has_bonus:
                    print(f"\n  All actions spent — auto-ending {pc_name}'s turn.")
                    end_this_turn = True
                else:
                    remaining = []
                    if has_action: remaining.append("Action")
                    if has_bonus: remaining.append("Bonus Action")
                    if not pc_fighter.reaction_used: remaining.append("Reaction")
                    print(f"\n  Remaining this turn: {', '.join(remaining)}")
                    print(f"  (Submit another action or 'end turn' to advance)")

            if end_this_turn:
                # Check combat end before processing NPC turns
                end_reason = _check_combat_end(combat, pc_name)
                if end_reason:
                    print(f"\n  {'=' * 40}")
                    if end_reason == "ALL_ENEMIES_DEAD":
                        ...
                        for n, f in combat.fighters.items():
                            if f.disposition in ("hostile", "") and not f.alive:
                                total_xp += 700  # Phase Spider XP; TODO: per-monster
                        if total_xp:
                            print(f"  XP earned: {total_xp}")
                    elif end_reason == "PC_DOWN":
                        print(f"  >>> {pc_name} IS DOWN — begin death saves <<<")
                    elif end_reason == "PARTY_WIPED":
                        print(f"  >>> TOTAL PARTY KILL <<<")
                    print(f"  {'=' * 40}")

                    if end_reason == "ALL_ENEMIES_DEAD":
                        _clear_combat_state(character)
                        print(f"  Combat state cleared.")
                        combat = None  # Skip dashboard reprint
                else:
                    # Process NPC turns until it's the PC's turn again
                    print(f"\n  {'─' * 50}")
                    print(f"  Advancing through NPC turns...")
                    print(f"  {'─' * 50}")
                    npc_turns = _process_npc_turns(combat, pc_name)
                    _print_npc_turn_results(npc_turns)

                    # After NPC turns, it should be PC's turn again
                    end_reason = _check_combat_end(combat, pc_name)
                    if end_reason:
                        print(f"\n  {'=' * 40}")
                        if end_reason == "ALL_ENEMIES_DEAD":
            ...
            # Save updated state
            if combat and combat.active:
                _save_combat_state(combat, character, combat_meta)

        # Print current dashboard (whether we just processed or not)
        if combat and combat.active:
            if 0 <= combat.turn_idx < len(combat.order):
                current_name = combat.order[combat.turn_idx][1]
            else:
                current_name = ""
            _print_combat_dashboard(combat, current_name)

            if current_name and current_name == pc_name:
                f = combat.fighters[current_name]
                print(f"\n  ╔══ D&D 5e ACTION ECONOMY — {current_name}'s TURN ══╗")
                print(f"  ║ {f.action_economy_line()}")
                print(f"  ║ Rules:")
                print(f"  ║  - 1 Action (Attack OR Cast a spell — NOT both)")
                print(f"  ║  - 1 Bonus Action (if available: Protector's Surge, etc.)")
                print(f"  ║  - 1 Reaction (opportunity attack, etc.)")
                print(f"  ║  - Movement: {f.movement_remaining}ft")
                print(f"  ║ A spell with casting time '1 action' USES your Action.")
                print(f"  ║ A spell with casting time '1 bonus action' USES your Bonus Action.")
                print(f"  ║ You CANNOT cast an action spell AND attack in the same turn")
                print(f"  ║ unless you have Haste or Action Surge granting a second Action.")
                print(f"  ╚{'═' * 55}╝")
        ...
        if wants_combat:
            ...
            # Detect monsters — simple keyword substring matching
            monsters_to_add = []
                ...
                if any(kw in action_lower for kw in keywords):
                    ...
                    for word in action_lower.split():
                        if word.isdigit():
                            count = int(word)
                            break
                    # Plural detection: "spiders" → more than 1
                    if count == 1:
                        for kw in keywords:
    ...
    # == CARDINAL RULES — LAST CHECK BEFORE AI OUTPUT ==
    pc = eng.char_name
    print(f"\n{'=' * 60}")
    print("CARDINAL RULES — OBEY BEFORE EVERY DM RESPONSE")
    print(f"{'=' * 60}")
    print(f"""
  RULE 1: NEVER SPEAK FOR {pc.upper()}
    The DM NEVER writes {pc}'s dialogue. Not in quotes. Not
    paraphrased. Not summarized. Not implied through narration.
    NEVER write {pc} taking actions the player didn't declare.
    If the scene needs {pc} to speak -> STOP and ask the player.
    ONLY exception: player intent is crystal clear AND words
    carry zero narrative weight ("I order a room" -> "You ask
    for a room"). Negotiation, emotion, strategy -> STOP.
    TEST: Read your response. Find every instance where {pc}
    talks, argues, explains, or responds verbally. DELETE ALL
    OF THEM. Present the NPC side and wait.

  RULE 2: NEVER AUTO-RESOLVE {pc.upper()}'S COMBAT
    All combat is round-by-round with player input EVERY round.
    Even "fight them" or "attack" -> set scene, roll initiative,
    describe Round 1, STOP. Player chooses every action.
    NEVER decide {pc}'s attacks, spells, reactions, movement,
    targets, or tactical approach. Not even "obvious" choices.

  RULE 3: STOP AT DECISION POINTS
    When {pc} must choose (fight/talk/move/plan/react) -> STOP.
    When an NPC asks {pc} a direct question -> STOP.
    When combat begins -> STOP after Round 1 setup.
    When something unexpected happens -> STOP.
    Present situations. Never assume the response.

  RULE 4: NO FABRICATED EXPOSITION NPCs
    Every named NPC must exist in character_tracker.md OR be a
    disposable background figure (unnamed, transactional only).
    No mysterious strangers. No convenient sages.

  RULE 5: DIALOGUE-DRIVEN NARRATIVE
    Scenes with NPCs present: at least 60% dialogue by volume.
    Information through conversation, not narration.
    Rhythm: talk -- beat -- talk -- beat -- talk.

  DM SELF-CHECK (run before EVERY response):
    1. Did I write dialogue for {pc}? -> DELETE IT
    2. Did I resolve combat without player input? -> REWRITE
    3. Did I decide {pc}'s actions/tactics? -> REWRITE
    4. Did I skip a decision point? -> REWRITE
    5. Is the response 60%+ dialogue in NPC scenes? -> REWRITE
""")
