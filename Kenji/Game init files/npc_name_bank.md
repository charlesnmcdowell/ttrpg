# NPC Name Bank — TTRPG Universe
> Pull names from here when creating new NPCs. Cross out or mark used names with campaign tag.
> **RULE:** All new NPC names MUST come from this bank. If a section is empty, add 10 new names before using one. Check across ALL campaigns before using a name.

---

## HUMAN

**Male:**
1. ~~Cedwyn~~ *(used — General, Bracken's former CO)*
2. ~~Torren~~ *(used — Cookie campaign, textile merchant at Gilt Lily, Varenholm)*
3. ~~Castor~~ *(used — Cookie campaign, turnip farmer outside Varenholm, Ch3)*
4. ~~Davan~~ *(used — Cookie campaign, adventurer guild member, greatsword fighter, Varenholm, Ch5)*
5. ~~Edric~~ *(used — Cookie campaign, Alderman Edric Tosse, city official, Varenholm, Ch3)*
6. Falconer
7. ~~Garron~~ *(used — Cookie campaign, adventurer crossbowman, bandit camp raid party)*
8. Selwyn
9. ~~Ivor~~ *(used — Cookie campaign, Academy student, lute player, Team Two, Ch4)*
10. Jareth

**Female:**
1. ~~Aveline~~ *(used — Cookie campaign, adventurer guild member, archer/scout, Varenholm, Ch5)*
2. ~~Brynn~~ *(used — Shield infantry, Thornkeep garrison)*
3. ~~Calista~~ *(used — barmaid **The Brass Hitch**, Bronzebarrow Ashmere 34)*
4. ~~Dorith~~ *(used — Cookie campaign, adventurer healer, bandit camp raid party)*
5. ~~Eira~~ *(used — Cookie campaign, recruiter for Lyssa Vane's troupe)*
6. ~~Fenella~~ *(used — Cookie campaign, Academy student, shortbow, Team Two, Ch4)*
7. ~~Gwynn~~ *(used — halfling hostler **The Brass Hitch**, Bronzebarrow — halfling; pulled from Human list per bank note)*
8. ~~Hadley~~ *(used — Watch Captain, Millhaven garrison)*
9. ~~Isolde~~ *(used — Cookie campaign, enchantress at The Pale Lantern, Varenholm, Ch3)*
10. ~~Jessamine~~ *(used — Cookie campaign, adventurer druid, bridge troll quest party, Ch5)*

---

## ORC

**Male:**
1. Borgakh
2. Dushnak
3. Ghorzul
4. Krazag
5. Mograth
6. Nazgur
7. Orshen
8. Rugdak
9. Thokk
10. Urzog

**Female:**
1. Azhra
2. Bolgra
3. Drukha
4. Gashna
5. Hurza
6. Krazhka
7. Mursha
8. Orgha
9. Shagra
10. Tulga

---

## DWARF

**Male:**
1. Barik
2. Dolgrim
3. Fundrin
4. Brondak
5. Hjalnir
6. Korvar
7. Maldren
8. Norik
9. Rurik
10. Thaldrin

**Female:**
1. Beldra
2. Dagny
3. Elgra
4. Fyra
5. Gerta
6. ~~Haldra~~ *(used — dwarf factor clerk **The Brass Hitch**, Bronzebarrow)*
7. Kettil
8. Myrna
9. Sigra
10. Tova

---

## UNDEAD

**Male:**
1. Ashcroft
2. Bleakhart
3. Valthen
4. Drevane
5. Wyscroft
6. Grimshaw
7. Holloway
8. Lannek
9. Morwen
10. Drathek

**Female:**
1. Ashwyn
2. Blythe
3. Carragh
4. Dusken
5. Elowen
6. Fennara
7. Grael
8. Ravenne
9. Luthiel
10. Veshra

---

## HALFLING

**Male:**
1. ~~Tomas~~ *(used — Cookie campaign, innkeeper Tomas Wren, Kettlebrook)*
2. Bramble
3. Corbin
4. Dillo
5. Emmet
6. Fisk
7. Garrick
8. Hobson
9. Liddle
10. Pip *(reserved — Kenji campaign, Pip the innkeeper)*

**Female:**
1. ~~Daisy~~ *(used — Cookie campaign, Cookie's mother, Kettlebrook)*
2. Clover
3. Dottie
4. Ember
5. Gilly
6. Hazel
7. Juniper
8. Lavender
9. Primrose
10. Rosie

---

## ELF

**Male:**
1. Aelindor
2. Caelith
3. Drevaris
4. Faelon
5. Ithrandir
6. Lorithan
7. Naeleth
8. Silvanthor
9. Thalion
10. Vaerun

**Female:**
1. Aelindra
2. Caelwen
3. Dawneth
4. Elariel
5. Faelith
6. Ilmaris
7. Lirael
8. Mirathel
9. Sylvaine
10. Vaelith

---

## INVENTED OUTSIDE BANK — Cross-Campaign Registry
> Names created during campaign generation that weren't pulled from the bank above.
> Logged here to prevent future collisions. Do NOT reuse these names.

**Cookie campaign:**
- Lyssa Vane *(human female — NE antagonist, enchantress/bard)*
- Cadence Ashworth *(human female — LG mentor, Varenholm Academy professor)*
- Silka *(human Ankuspawn female — TN, charmed spy at Academy, Cookie's half-sister, Ember: Veil suppressed by Lyssa's amulet)*
- Rook *(human male — CE enforcer for Lyssa Vane)*
- Fern *(human female — CG, Cookie's best friend from Kettlebrook)*
- Maeven *(human, tailor on Chandler's Row, Varenholm)*
- Gruff *(human male — LN, Adventurer's Guild clerk, Varenholm)*
- Dalla *(human female — TN, adventurer fighter)*
- Silas *(human male — CN, adventurer rogue)*
- Marta *(human female — NG, adventurer ranger)*
- Edric Tosse *(human male — LN, alderman/city official, Varenholm)* [first name from bank, surname invented]
- Maret *(human female — NG, Halbert's widow, Brewer's Row canal district, Varenholm, Ch6)*
- Halbert *(human male — NG, deceased dockworker/crane operator, Varenholm, Ch6)*
