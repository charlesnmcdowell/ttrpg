...
def create_campaign_folder(name: str, campaign: dict) -> Path:
    ...
    Also writes a central manifest into:
        Kenji/Game init files/manifests/<lower-name>.json
    so the new character can be launched via:
        Kenji DM Tool.exe --character <lower-name>
    All paths in the manifest are TTRPG-root-relative so the build is portable.
    """
    ...
    # ---- Central manifest (Kenji/Game init files/manifests/<name>.json) ----
    manifests_dir = SCRIPT_DIR / "manifests"
    manifests_dir.mkdir(parents=True, exist_ok=True)

    manifest_id = folder_name.lower()
    manifest_path = manifests_dir / f"{manifest_id}.json"

    # All paths TTRPG-root-relative — kenji_gui resolves them against the root.
    manifest = {
        "_comment": (
            f"{name} campaign — auto-generated by generate_starter_campaign.py. "
            "Shares engine + music with Kenji install. "
            "All paths are relative to the TTRPG root."
        ),
        "campaign_id": manifest_id,
        "window_title_template": "{char_name} — Live Dashboard",
        "state_file": f"{folder_name}/Game init files/character_world_state.json",
        "music_dir": "Kenji/music",
        "music_map": "Kenji/music/music_map.json",
        "engine_path": "Kenji/Game init files",
    }

    # Don't clobber an existing manifest unless it's identical (idempotent re-runs).
